...
1. Copy `codex-skills/contract-review` into `$CODEX_HOME/skills/contract-review`
2. Restart Codex
...
고위험 조항과 검토 의견서에는 아래 4개 레이어를 함께 출력합니다.

1. **결론** — 어떤 리스크인지
2. **판단 기준** — 어떤 법리/실무 기준으로 그렇게 봤는지
3. **원천 소스** — 법령, 조문, 판례 경향, 표준계약서/실무 기준
4. **불확실성** — 추가 확인이 필요한 사실관계 또는 최신성 한계

원천 소스는 가능하면 아래 단위까지 적습니다.

- 법령: 법률명 + 조/항/호
- 판례: 법원 + 선고일 + 사건번호
- 시장 관행: 어떤 표준계약서/실무 기준인지
- 확인일: 검토 기준 시점
...
1. `codex-skills/contract-review` 폴더를 `$CODEX_HOME/skills/contract-review`로 복사
2. Codex 재시작
